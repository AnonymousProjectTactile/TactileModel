...
from tactile_gym.rl_envs.exploration.edge_follow.rest_poses import (  rest_poses_dict,)


env_modes_default = {
    'movement_mode': 'xyz',
    # 'control_mode': 'TCP_position_control' ,# 'TCP_velocity_control',
    'control_mode': 'TCP_velocity_control',
    'noise_mode': 'fixed_height',
    # 'observation_mode': 'oracle',
    'observation_mode': 'oracle_tactile',
    'reward_mode': 'dense',
    'arm_type': 'ur5', # 'mg400',
    # 'arm_type': 'mg400', # 'mg400',
}
...
class EdgeFollowEnv(BaseTactileEnv):
    ...
    def dense_reward(self):
        ...
        # reward = -((W_goal * goal_dist) + (W_edge * edge_dist) + (W_yaw * yaw_dist)) # sensor到目标的距离负值 + 
    ...
    def get_oracle_obs(self):
        ...
        observation = np.hstack(
            [
                *tcp_pos_workframe, # 3
                *tcp_lin_vel_workframe, # 3,
                *self.goal_pos_workframe, # 3, 
                self.edge_ang, # 1
            ]
        )
        return observation
