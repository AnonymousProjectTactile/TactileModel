...
def train_agent(algo_name="ppo",env_name="edge_follow-v0", rl_params={}, algo_params={}, augmentations=None,):
    ...
    eval_env = make_eval_env(  env_name, rl_params,show_gui=False,show_tactile=False, )
    ...
    # test env 
    # obs = env.reset()
    # obs_tactile = obs["tactile"] # [10,1 128,128]
    # action = env.action_space.sample()  # 从动作空间中随机选取一个动作  (2,)
    # action = np.array([0, 0.0072, 0.015])
    # obs1 = env.step(action)
    # plt.figure()
    # plt.imshow(obs_tactile[0][0])
    # plt.show()

    # train an agent
    with ProgressBarManager(rl_params["total_timesteps"]) as progress_bar_callback:
        model.learn(   total_timesteps=rl_params["total_timesteps"], callback=[progress_bar_callback, eval_callback, event_plotting_callback],)
